...
import { createInlineProgress, createFileCard, getFileTypeStyle } from "@/lib/design-system";
...
export function ModuleCard({ 
  ...
  const progressPercentage = validMaterials.length > 0 ? (completedMaterials / validMaterials.length) * 100 : 0;
  const progressData = createInlineProgress(progressPercentage);
  
  // Create progress pill text
  const progressPill = validMaterials.length === 0 
    ? "No files" 
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <span className="text-sm text-gray-500 font-medium">
              {validMaterials.length} {validMaterials.length === 1 ? 'file' : 'files'}
            </span>
          ...
          {validMaterials.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm text-gray-600 text-center max-w-md mb-4 leading-relaxed">
                Drop PDF, audio, video, or presentation files here to get started with AI-powered learning experiences
              </p>
              
              <div className="flex items-center gap-2 text-xs text-gray-500">
                <span className="px-2 py-1 bg-gray-200 rounded-full font-medium">PDF</span>
                <span className="px-2 py-1 bg-gray-200 rounded-full font-medium">Audio</span>
                <span className="px-2 py-1 bg-gray-200 rounded-full font-medium">Video</span>
                <span className="px-2 py-1 bg-gray-200 rounded-full font-medium">Docs</span>
              </div>
            </div>
          ) : (
