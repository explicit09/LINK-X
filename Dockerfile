...
# Copy requirements from docker-image/src directory
COPY docker-image/src/requirements.txt .
COPY docker-image/src/requirements-migration.txt .
...
# Copy application code from docker-image
COPY docker-image/ .
...
CMD ["gunicorn", "--bind", "0.0.0.0:8000", "--workers", "4", "--threads", "2", "--timeout", "120", "--chdir", "src", "app:app"]