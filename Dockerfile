...
RUN --mount=type=cache,id=learnx-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=learnx-apt-lib,target=/var/lib/apt,sharing=locked \
...
RUN --mount=type=cache,id=learnx-pip-cache,target=/root/.cache/pip \
...
RUN --mount=type=cache,id=learnx-apt-cache-runtime,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=learnx-apt-lib-runtime,target=/var/lib/apt,sharing=locked \
    apt-get update && apt-get install -y --no-install-recommends \
    libpq5 \
    curl \
