...
@app.route('/api/v1/auth/me', methods=['PATCH'])
def me_patch():
    session = get_user_session()
    if 'error' in session:
        return jsonify(session), 401
...
@app.route('/api/v1/auth/me', methods=['DELETE'])
def me_delete():
    session = get_user_session()
    if 'error' in session:
        return jsonify(session), 401
...
@app.route('/api/v1/auth/register/instructor', methods=['POST'])
...
@app.route('/api/v1/auth/register/student', methods=['POST'])
...
# Generic courses endpoint that works for both students and instructors
...
def api_courses():
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            description = data.get('description', '')
            course = create_course(db, title, description, user.id)
...
# Todo endpoints
@app.route('/api/v1/todo-items', methods=['GET', 'POST'])
def api_todo_items():
    session = get_user_session()
    if 'error' in session:
        return jsonify(session), 401
...
@app.route('/api/v1/auth/sessionLogin', methods=['POST'])
def session_login():
    data = request.get_json() or {}
    id_token = data.get('idToken')
    if not id_token:
...
@app.route('/api/v1/auth/sessionLogout', methods=['POST'])
def session_logout():
