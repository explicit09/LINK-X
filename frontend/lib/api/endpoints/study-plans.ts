...
export const studyPlansAPI = {
  ...
  listPlans: (includeInactive = false) =>
    apiClient.get<StudyPlan[]>(`/study-plans?include_inactive=${includeInactive}`),
  ...
  getActivePlan: () =>
    apiClient.get<StudyPlan>('/study-plans/active'),

  // Goal endpoints
  listGoals: (params?: { status?: string; priority?: string; limit?: number }) => {
    ...
    return apiClient.get<StudyGoal[]>(`/study-plans/goals${query ? `?${query}` : ''}`);
  },
  ...
  // Session endpoints
  listSessions: (params?: { limit?: number; start_date?: string; end_date?: string }) => {
    const searchParams = new URLSearchParams();
    if (params?.limit) searchParams.set('limit', params.limit.toString());
    ...
    const query = searchParams.toString();
    return apiClient.get<StudySession[]>(`/study-plans/sessions${query ? `?${query}` : ''}`);
  },
  ...
  endSession: (sessionId: string, data: EndSessionData) =>
    apiClient.post<StudySession>(`/study-plans/sessions/${sessionId}/end`, data),

  // Recommendation endpoints
  listRecommendations: (limit?: number) => {
    const query = limit ? `?limit=${limit}` : '';
    return apiClient.get<StudyRecommendation[]>(`/study-plans/recommendations${query}`);
  },
