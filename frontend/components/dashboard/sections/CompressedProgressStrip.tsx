...
export function CompressedProgressStrip({
  ...
  const xpProgress = stats.xpToNextLevel > 0 ? (stats.currentXP / stats.xpToNextLevel) * 100 : 0;
    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-3 mb-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        >
          ...
          <span className="text-sm font-semibold text-orange-700">{stats.dailyStreak}-Day</span>
        </div>
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-xs text-gray-600">Level {stats.currentLevel}</span>
            <span className="text-xs text-purple-600 font-medium">{xpToGo} XP to go</span>
          </div>
          <div className="w-16 bg-gray-200 rounded-full h-1.5">
            <div 
              className={cn("h-1.5 rounded-full transition-all duration-500 bg-gradient-to-r", getLevelColor(stats.currentLevel))}
        ...
        <div className="flex items-center space-x-2 bg-green-50 border border-green-200 rounded-full px-3 py-1.5">
          ...
          <span className="text-sm font-semibold text-green-700">{stats.todayCompleted}/{stats.weeklyGoal}</span>
        ...
        <div className={cn(
          ...
          justEarnedXP && "animate-pulse bg-yellow-100 border-yellow-300 scale-105"
        )}>
          <Zap className={cn("h-4 w-4 text-yellow-600", justEarnedXP && "animate-bounce")} />
          <span className="text-sm font-semibold text-yellow-700">+{stats.todayXP} XP</span>
          <span className="text-xs text-yellow-600">Today</span>
          {justEarnedXP && (
            <span className="text-xs text-yellow-800 font-bold animate-fade-in">New!</span>
          )}
        </div>
