              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col items-start">
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge variant="secondary">{todaySchedule.length} events</Badge>
                  </div>
                  <div className="text-sm text-gray-500 mt-1">
                    Next: CS229 @ 9am • {todaySchedule.filter(e => e.status !== "completed").length} remaining
                  </div>
                </div>
                {isScheduleExpanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
              </div>
            </CardHeader>
          </CollapsibleTrigger>
          <CollapsibleContent>
                ...
                {todaySchedule.map((event, index) => (
                  <div key={index} className="flex items-center justify-between p-2 rounded hover:bg-gray-50">
                    <div className="flex items-center space-x-3">
                      <span className="text-sm font-medium text-gray-600 w-16">{event.time}</span>
                      <span className="text-sm text-gray-900">{event.title}</span>
                    </div>
                    <Badge 
                      variant={event.status === "urgent" ? "destructive" : event.status === "completed" ? "default" : "secondary"}
                      className="text-xs"
                    >
                      {event.status}
                    </Badge>
            ...
            <CardHeader className="cursor-pointer hover:bg-gray-50 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex flex-col items-start">
                  ...
                  <div className="text-sm text-gray-500 mt-1">
                    Avg: {Math.round(courses.reduce((acc, c) => acc + c.progress, 0) / courses.length)}% complete • 1 behind schedule
                  </div>
                </div>
            ...
            <CardContent className="pt-0">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {courses.map((course) => (
                  ...
                  >
                    <h4 className="font-medium text-gray-900 text-sm">{course.code}</h4>
