...
import { useTodaySchedule, usePerformancePulse, useCoursesOverview } from "@/hooks/useDashboardData";
import { 
  Calendar, 
  Clock,
  ChevronRight,
...
export function DashboardSidebar({
  ...
  const { data: performanceData, loading: performanceLoading } = usePerformancePulse();
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`text-gray-900 ${event.is_next ? "font-medium" : ""}`}>
                      {event.title}
                    </span>
                  </div>
                  <Badge 
      ...
      <Card className="border border-gray-200">
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center space-x-2 text-base">
            ...
            {performanceLoading && <Loader2 className="h-3 w-3 animate-spin text-gray-400" />}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {performanceLoading ? (
            ...
            <div className="space-y-3">
              <div className="text-center">
                <div className={`text-2xl font-bold ${getImprovementColor(performanceMetrics.improvement_percentage)}`}>
                  {formatImprovementDisplay(performanceMetrics.improvement_percentage)}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-center">
                <div className="p-2 bg-amber-50 rounded border border-amber-200">
                  <div className="text-sm font-semibold text-amber-700">
                    {performanceMetrics.current_rank > 0 ? `Rank #${performanceMetrics.current_rank}` : "Unranked"}
                  </div>
                  <div className="text-xs text-amber-600">
                    {performanceMetrics.rank_change > 0 ? `+${performanceMetrics.rank_change} this week` : 
                     performanceMetrics.rank_change < 0 ? `${performanceMetrics.rank_change} this week` : "No change"}
                  </div>
                </div>
      ...
      <Card className="border border-gray-200 lg:order-3 order-last">
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center space-x-2 text-sm font-semibold">
            <span>📚</span>
            <span>Courses</span>
            {coursesLoading && <Loader2 className="h-3 w-3 animate-spin text-gray-400" />}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {coursesLoading ? (
              ...
              <div className="flex items-center gap-2 text-xs">
                <div className="p-1.5 bg-green-50 rounded border border-green-200">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium text-green-700">{coursesOverview.active_courses} Active</span>
                  </div>
                </div>
                ...
                {coursesOverview.behind_courses > 0 && (
                  <div className="p-1.5 bg-red-50 rounded border border-red-200">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-medium text-red-700">{coursesOverview.behind_courses} Behind</span>
                    </div>
                  </div>
                )}
                {/* Show celebration when all on track */}
                {coursesOverview.behind_courses === 0 && coursesOverview.active_courses > 0 && (
