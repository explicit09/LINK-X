...
import { Target, Zap, Clock, BarChart3, AlertTriangle, CheckCircle, TrendingUp } from "lucide-react";
...
export function CohesiveWeeklyMission({ 
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "behind": return <AlertTriangle className="h-4 w-4 text-orange-500" />;
  ...
  const getSmartCopy = (goal: WeeklyGoal) => {
    ...
    switch (goal.type) {
      case "xp":
        return remaining > 0 ? `Almost there! Earn ${remaining} more XP to reach ${goal.total}` : `🎉 ${goal.total} XP Complete!`;
      case "tasks":
        return remaining > 0 ? `${goal.current}/${goal.total} Tasks Complete — ${remaining} left!` : `🎉 All ${goal.total} Tasks Done!`;
      case "study":
        return remaining > 0 ? `Almost done: ${remaining} hours left` : `🎉 ${goal.total} Study Hours Complete!`;
      default:
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Badge className="bg-green-100 text-green-700 border-green-200 px-3 py-1">
          🧠 {goals.find(g => g.type === "tasks")?.current}/{goals.find(g => g.type === "tasks")?.total} Tasks Complete
        </Badge>
        <Badge className="bg-yellow-100 text-yellow-700 border-yellow-200 px-3 py-1">
          🎯 {goals.find(g => g.type === "xp")?.current}/{goals.find(g => g.type === "xp")?.total} XP
        </Badge>
        <Badge className="bg-purple-100 text-purple-700 border-purple-200 px-3 py-1">
          ⏳ {goals.find(g => g.type === "study")?.current}/{goals.find(g => g.type === "study")?.total}h Study
        </Badge>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">This Week's Milestones</h3>
            {behindGoals > 0 && (
              ...
              >
                <div className="flex items-center space-x-3 flex-1">
                  <div className={`p-2 rounded-full ${
                    goal.type === 'xp' ? 'bg-yellow-100' : 
                    goal.type === 'tasks' ? 'bg-blue-100' : 'bg-purple-100'
                  }`}>
                    {getIcon(goal.type)}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span className="text-sm text-gray-600">
                        {goal.current}/{goal.total} {goal.type === "study" ? "Hours" : goal.type === "xp" ? "XP" : "Complete"}
                      </span>
                      {getStatusIcon(goal.status)}
                    </div>
                    <p className="text-xs text-gray-600">{getSmartCopy(goal)}</p>
                  </div>
                </div>
