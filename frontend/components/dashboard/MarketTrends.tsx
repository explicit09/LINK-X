...
async function fetchRecentMarketPricesDirectly() {
  try {
    const res = await fetch('http://localhost:8000/market/recent', {
      method: 'GET',
    });
...
export default function MarketTrends() {
  ...
  const renderChart = () => {
    ...
    const pathData = marketPrices
      .map((d, i) => {
        const scaledX =
          ((d.date.getTime() - minDate) / (maxDate - minDate)) * 300;
  ...
  return (
    <div
      className={cn(
        ...
        isExpanded
          ? 'fixed inset-0 bg-black z-50 flex items-center justify-center p-6'
          : 'relative',
      )}
    >
      <Card
