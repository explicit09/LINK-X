...
export function PerformanceMetrics({
  ...
  const frameRate =
    frameStats.total > 0
      ? Math.round(
          ((frameStats.total - frameStats.dropped) / frameStats.total) * 60,
        )
      : 60;
      ...
      <CardContent className="space-y-4">
        ...
        {memoryUsage > 0 && (
          <div className="space-y-2">
            ...
            <Progress
              value={Math.min((memoryUsage / 500) * 100, 100)}
              className="h-2"
            />
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              ...
              <p className="font-medium">
                {aggregateMetrics.avgFirstToken.toFixed(0)}ms
              </p>
            </div>
            <div>
              <p className="text-muted-foreground">Avg Completion</p>
              <p className="font-medium">
                {(aggregateMetrics.avgCompletion / 1000).toFixed(1)}s
              </p>
            </div>
            <div>
