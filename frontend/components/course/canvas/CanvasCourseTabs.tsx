...
export function CanvasCourseTabs({ courseId, userRole, className }: CanvasCourseTabsProps) {
  ...
  const isTabActive = (tab: Tab) => {
    if (tab.id === 'home') {
      // Home tab is active when we're at the exact course URL
      return pathname === `/courses/${courseId}`;
    }
    // Other tabs are active when the pathname includes their href
  ...
  return (
    <div className={cn("bg-white border-b border-gray-200 sticky top-0 z-40", className)}>
      <div className="px-6">
