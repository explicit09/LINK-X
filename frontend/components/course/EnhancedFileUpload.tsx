...
export function EnhancedFileUpload({ courseId, moduleId, userRole = 'student', onUploadComplete, className }: EnhancedFileUploadProps) {
  ...
  const studentUpload = async (uploadFile: UploadFile) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v2/files/upload`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const instructorUpload = async (uploadFile: UploadFile) => {
    ...
    try {
      ...
      let moduleId = null;
      try {
        const modulesResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v2/courses/${courseId}/modules`, {
          method: 'GET',
          credentials: 'include',
        });
      ...
      if (!moduleId) {
        try {
          const createModuleResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v2/courses/${courseId}/modules`, {
            method: 'POST',
            headers: {
