...
interface UploadServiceOptions {
  ...
  onStatusChange: (fileId: string, status: UploadFile['status'], stage?: string) => void;
...
export class UploadService {
  ...
  private async studentUpload(uploadFile: UploadFile) {
    ...
    try {
      ...
      sonnerToast.error(`Failed to upload ${uploadFile.file.name}: ${errorMessage}`);
    }
  }
  ...
  private async instructorUpload(uploadFile: UploadFile) {
    ...
    try {
      ...
      // Get course modules
      try {
        const modulesResponse = await fetch(`${API_URL}/api/v2/courses/${this.options.courseId}/modules`, {
          credentials: 'include'
        });

        if (!modulesResponse.ok && !this.options.moduleId) {
          // Create a default module if none exists
          const createModuleResponse = await fetch(`${API_URL}/api/v2/courses/${this.options.courseId}/modules`, {
            method: 'POST',
            headers: {
  ...
  private async simulateProgress(fileId: string, status: "uploading" | "processing") {
