...
export function EnhancedFileUpload({ 
  ...
  const { uploadFiles, addFile, updateFile, removeFile, getFile } = useFileUpload();
  ...
  const uploadService = new UploadService({
    ...
    onComplete: (fileId, result) => {
      updateFile(fileId, { status: 'completed', processingStage: 'Upload complete!' });
  ...
  const retryUpload = useCallback((fileId: string) => {
    ...
    if (uploadFile) {
      updateFile(fileId, { status: "uploading", progress: 0, error: undefined });
      uploadService.uploadFile(uploadFile, userRole);
    }
