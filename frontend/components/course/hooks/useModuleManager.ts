...
export function useModuleManager(
  ...
  const updateModule = useCallback(
    async (moduleId: string, updates: Partial<Pick<Module, 'title'>>) => {
      try {
          ...
          prev.map((module) =>
            module.id === moduleId
              ? {
                  ...module,
                  ...updates,
                  editTitle: updates.title || module.editTitle,
                }
              : module,
          ),
        );
  ...
  const updateEditTitle = useCallback((moduleId: string, title: string) => {
    setModules((prev) =>
      prev.map((module) =>
        module.id === moduleId ? { ...module, editTitle: title } : module,
      ),
    );
  }, []);

  // Add material to module
  const addMaterialToModule = useCallback(
    (moduleId: string, material: Material) => {
      setModules((prev) =>
        prev.map((module) =>
          module.id === moduleId
  ...
  const removeMaterialFromModule = useCallback(
