...
export function useFileUpload(
  courseId: string,
  userRole: 'instructor' | 'student' = 'student',
) {
    ...
    async (
      ...
      try {
        ...
        return new Promise((resolve, reject) => {
          ...
          xhr.open(
            'POST',
            `/api/courses/${courseId}/modules/${moduleId}/upload`,
          );
          xhr.send(formData);
        });
      } catch (err) {
  ...
  const handleDragEnter = useCallback(
    (moduleId: string, event: React.DragEvent) => {
      event.preventDefault();
  ...
  const handleDrop = useCallback(
    (moduleId: string, event: React.DragEvent, options?: FileUploadOptions) => {
      event.preventDefault();
      setDragStates((prev) => new Map(prev).set(moduleId, false));
  ...
  const setFileInputRef = useCallback(
    (moduleId: string, ref: HTMLInputElement | null) => {
      if (ref) {
  ...
  const getUploadProgress = useCallback(
    (moduleId: string): UploadProgress | null => {
  ...
  const isModuleUploading = useCallback(
  ...
  const isModuleDragging = useCallback(
    (moduleId: string): boolean => {
