...
export const DropZone: React.FC<DropZoneProps> = ({
  ...
  const iconColor = isDragOver 
    ? (isPackage ? "text-orange-600" : "text-blue-600")
    : "text-gray-400";
  const borderColor = isDragOver
    ? (isPackage ? "border-orange-400 bg-orange-50" : "border-blue-400 bg-blue-50")
    ...
    <>
      ...
      >
        <CardContent className="p-8 text-center">
          ...
          <Button className="mt-4" size="sm" variant={isPackage ? "secondary" : "default"}>
            {isPackage ? <Archive className="h-4 w-4 mr-2" /> : <FileText className="h-4 w-4 mr-2" />}
