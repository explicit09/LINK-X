...
export const useSettings = () => {
  ...
  // Update account info
  const updateAccount = useCallback(async (accountData: AccountData): Promise<void> => {
    try {
      const res = await fetch(`${API_URL}/me`, {
        method: "PATCH",
        credentials: "include",
  ...
  const updateOnboarding = useCallback(async (onboardingData: OnboardingData): Promise<void> => {
    const payload = {
