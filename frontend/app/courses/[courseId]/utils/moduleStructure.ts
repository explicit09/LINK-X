...
export const createModuleStructure = (
    ...
    const defaultModules = [
      {
        ...
        description: 'Materials uploaded by students',
      },
      {
        id: 'week-1',
        title: 'Week 1',
  ...
  materials.forEach((material) => {
    if (material.moduleId && moduleMap.has(material.moduleId)) {
      ...
      console.warn(
        `Material ${material.title} has moduleId ${material.moduleId} but module not found in moduleMap`,
      );
  ...
  if (courseId) {
    try {
      ...
      if (savedState) {
        const accordionState = JSON.parse(savedState) as Record<
          string,
          boolean
        >;
...
export const filterMaterials = (
  ...
  return materials.filter((material) => {
    ...
    if (
      ...
      !filters.fileTypes.includes(material.type)
    ) {
      return false;
    }
