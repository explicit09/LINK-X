...
export function PerformanceMetricsPanel({
  ...
  const avgFirstToken =
    allMetrics
      .filter(([_, m]) => m.firstTokenTime)
      .reduce((acc, [_, m]) => acc + (m.firstTokenTime || 0), 0) /
      allMetrics.filter(([_, m]) => m.firstTokenTime).length || 0;
  const p95Completion =
    allMetrics
      .filter(([_, m]) => m.completionTime)
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="space-y-2 text-xs">
          <div className="flex justify-between">
            ...
            <span
              className={`font-mono ${getColor('firstToken', avgFirstToken)}`}
            >
          ...
          <div className="flex justify-between">
            ...
            <span
              className={`font-mono ${getColor('completion', p95Completion)}`}
            >
          ...
          <div className="flex justify-between">
            ...
            <span
              className={`font-mono ${getColor('frames', (frameStats.dropped / frameStats.total) * 100)}`}
            >
              {frameStats.total > 0
                ? ((frameStats.dropped / frameStats.total) * 100).toFixed(1)
                : 0}
              %
            </span>
          </div>
