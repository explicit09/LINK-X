...
export function useStreamingContent(
  ...
  const [streamingStates, setStreamingStates] = useState<
    Map<string, 'waiting' | 'streaming' | 'complete'>
  >(new Map());
  ...
  const metricsRef = useRef<
    Map<
      string,
      { startTime: number; firstTokenTime?: number; completionTime?: number }
    >
  >(new Map());
  ...
  // Calculate progress
  const totalSections =
    outline?.chapters.reduce((acc, ch) => acc + ch.subsections.length, 0) || 0;
  const completedCount = Array.from(streamingStates.values()).filter(
    (state) => state === 'complete',
  ).length;
  const streamingCount = Array.from(streamingStates.values()).filter(
  ...
  const streamSection = useCallback(
    async (
      ...
      if (
        ...
        (!regenerate && currentState === 'complete')
      ) {
        return;
      }
...
Happy learning!`;
          ...
          for (let i = 0; i < mockChunks.length; i++) {
            ...
            setStreamingContent((prev) =>
              new Map(prev).set(sectionKey, currentContent),
            );
          ...
          setStreamingStates((prev) =>
            new Map(prev).set(sectionKey, 'complete'),
          );
          ...
          // Real API call
          const response = await fetch(
            `http://localhost:8080/api/v2/files/${fileId}/stream-section`,
            {
              method: 'POST',
              headers: {
          ...
          let accumulatedContent = regenerate
            ? ''
            : streamingContent.get(sectionKey) || '';
              ...
              if (line.startsWith('data: ')) {
                try {
                  ...
                  if (data.content) {
                    ...
                    setStreamingContent((prev) =>
                      new Map(prev).set(sectionKey, accumulatedContent),
                    );
                  }
                } catch (error) {
        ...
        if (error.name === 'AbortError') {
          ...
          setStreamingStates((prev) =>
            new Map(prev).set(sectionKey, 'waiting'),
          );
  ...
  const regenerateSection = useCallback(
    (chapterId: string, subsectionId: string) => {
