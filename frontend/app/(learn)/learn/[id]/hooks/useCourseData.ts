    ...
    const fetchCourseData = async () => {
      try {
        const res = await fetch(
          `http://localhost:8080/student/personalized-files/${pfId}`,
          {
            credentials: 'include',
          },
        );
        ...
        if (data.content) {
          ...
          if (parsedContent.chapters) {
            const formattedChapters: Chapter[] = parsedContent.chapters.map(
              (ch: any, index: number) => ({
                chapterTitle: ch.chapterTitle,
                subsections: ch.subsections.map(
            ...
            const completed = formattedChapters.reduce(
              (acc, chapter) =>
                acc + chapter.subsections.filter((sub) => sub.completed).length,
              0,
            );
            ...
            for (let i = 0; i < formattedChapters.length; i++) {
              for (
                let j = 0;
                j < formattedChapters[i].subsections.length;
                j++
              ) {
