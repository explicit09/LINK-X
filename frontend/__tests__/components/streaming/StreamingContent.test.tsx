...
const StreamingTestHelper = ({ fileId }: { fileId: string }) => {
  ...
  return (
    <div>
      <button onClick={() => setActiveSection('section-1')}>
        Set Active Section
      </button>
      <button onClick={() => startStreaming('section-1')}>
        Start Streaming
      </button>
...
describe('StreamingContent', () => {
  ...
  describe('Rendering', () => {
    it('renders loading state when no active section', () => {
      ...
      expect(
        screen.getByText(/select a section to start learning/i),
      ).toBeInTheDocument();
    });

    it('renders streaming content when section is active', async () => {
      const mockCleanup = jest.fn();
      (api.streaming.streamLearningContent as jest.Mock).mockReturnValue(
        mockCleanup,
      );
  ...
  describe('Streaming Behavior', () => {
    ...
    it('handles streaming messages correctly', async () => {
      ...
      (api.streaming.streamLearningContent as jest.Mock).mockImplementation(
        (fileId, options, callback) => {
          ...
          setTimeout(() => {
            callback({
              type: 'content',
              data: 'Hello, this is streaming content!',
            });
          }, 10);
          return jest.fn();
        },
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Hello, this is streaming content!'),
        ).toBeInTheDocument();
      });
    });
    ...
    it('accumulates streamed content', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('First chunk. Second chunk. Third chunk.'),
        ).toBeInTheDocument();
      });
    });
